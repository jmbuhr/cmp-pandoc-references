...
local function get_bib_path(lines)
  ...
  if not initial_bib then
    if should_log() then
      vim.notify('cmp-pandoc-references: No bibliography file specification found in document', vim.log.levels.DEBUG)
    end
    return nil
  end
  ...
  if should_log() then
    vim.notify(string.format('cmp-pandoc-references: Bibliography file not found. Tried:\n- %s\n- %s', direct_path, full_path), vim.log.levels.WARN)
  end
  return nil
end
